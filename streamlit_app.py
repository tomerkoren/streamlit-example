...
with st.spinner(text=message.capitalize() + '...'):
    ...
    for row_i, row in enumerate(data_rows):
        ...
        if len(pairs) == 0:
            st.warning(f'Constraint in column 10, row {row_i+3} yielded 0 matches', icon="⚠️")
    ...
    for row_i, row in enumerate(data_rows):
        ...
        pairs = get_matching_pairs(pattern1,pattern2,exam_names,exam_index)
        if len(pairs) == 0:
            st.warning(f'Constraint in column 16, row {row_i+3} yielded 0 matches', icon="⚠️")
    ...
    for row_i, row in enumerate(data_rows):
        ...
        if len(matches) == 0:
            st.warning(f'Constraint in column 20, row {row_i+3} yielded 0 matches', icon="⚠️")
